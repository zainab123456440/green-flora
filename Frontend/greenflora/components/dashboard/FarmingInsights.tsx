...
function buildMarketInsight(commodity: MarketCommodity | null): string {
  ...
  // FIX: Swap "/" for " per " so Google Translate uses "فی" (fee)
  // to prevent RTL formatting bugs
  const price = formatPKRWithUnit(
  ...
  return (
    <div className="rounded-card border border-neutral-200 bg-surface-card p-4 shadow-card transition-all duration-200 hover:border-primary-200 hover:shadow-elevated">
      <div className="flex items-center justify-between gap-3">
        ...
        <Link
          href={href}
          className="shrink-0 text-xs font-medium text-primary-700 transition-colors hover:text-primary-900 hover:underline"
        >
          {linkLabel} →
        </Link>
      </div>
...
function WeatherInsightCard({
  ...
  const insightText =
    weatherLoading && hasLocation
      ? null
      : buildWeatherInsight(weather, hasLocation);
...
function MarketInsightCard({
  ...
  const insightText = marketLoading
    ? null
    : buildMarketInsight(commodity);
      ...
      <>
        ...
        <p className="mt-1 text-xs text-neutral-500">
          {commodity.latest_date
            ? formatMarketDate(commodity.latest_date)
            : "—"}{" "}
